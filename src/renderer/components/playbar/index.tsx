...
export const Playbar = memo(({ setFullscreen }: { setFullscreen: (arg: boolean) => void }) => {
  ...
  return (
    <div className="relative flex h-20 w-full shrink-0 overflow-hidden border-t-2 border-(--border-color)/10">
      ...
      <div className="grid h-full w-full grid-cols-3 gap-3">
        <NowPlaying index={index} queueLength={queue.length} track={state.current} setFullscreen={setFullscreen} />
