...
export const FXControls = memo(
  ({ id, repeatMode, canCF, setState }: { id: string | null; repeatMode?: boolean; canCF: boolean; setState: T[1] }) => {
      ...
      <div className="flex h-full w-full flex-row items-center justify-end gap-10 px-7">
        <div className="flex h-full w-fit flex-row items-center gap-5 pl-5">
          <div
            ...
            onClick={() => canCF && setFx((s) => ({ ...s, crossfeed: !s.crossfeed }))}
            className={"active:scale-85" + (canCF ? " cursor-pointer" : "") + (fx.crossfeed ? " text-(--accent-color)" : "")}
          />
          ...
          <RiMoonFill
            ...
            className={"cursor-pointer active:scale-85 " + (sleepTime ? "text-(--accent-color)" : "")}
          />
