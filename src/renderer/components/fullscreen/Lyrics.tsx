...
export const Lyrics = memo(({ lyrics }: { lyrics: Track["lyrics"] }) => {
  ...
  const parsed = useMemo(() => {
    try {
      if (/^<\?xml[\s\S]*?<tt\b/i.test(lyrics) || /<tt\b[^>]*>/i.test(lyrics)) return parseTTML(lyrics);
      else return parseLRC(lyrics);
    } catch {
      return { syncType: "None", leadingSilence: 0, lines: [] };
    }
  }, [lyrics]);

  const _progress = useMemo(() => progress + 1, parsed.syncType === "None" ? [] : [progress]);
  ...
  useEffect(
    () => {
      ...
      container.scrollTo({
        ...
        top: parsed.syncType === "None" ? 0 : el.offsetTop - (container.offsetTop + container.offsetHeight / 2.75),
      });
    },
    parsed.syncType === "None" ? [] : [parsed.lines.find((l) => _progress >= l.start / 1000 && _progress < (l.end / 1000 || Infinity))],
  );
      ...
      {lyrics !== "No lyrics found"
        ? [
            ...
            ...parsed.lines.map((l, i) => {
              const isActive = parsed.syncType !== "None" && _progress >= l.start / 1000 && _progress < (l.end / 1000 || Infinity);
              return (
                <div
                  key={i}
                  children={l.text}
